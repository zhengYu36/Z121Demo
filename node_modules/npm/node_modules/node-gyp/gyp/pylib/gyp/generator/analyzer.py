...
Consider a graph like the following:
 ...
 / \
B   C
ImageUtil depends upon both B and C, ImageUtil is of type none and B and C are executables.
D is an executable, has no dependencies and nothing depends on it.
If |additional_compile_targets| = ["ImageUtil"], |test_targets| = ["B", "C"] and
files = ["b.cc", "d.cc"] (B depends upon b.cc and D depends upon d.cc), then
the following is output:
|compile_targets| = ["B"] B must built as it depends upon the changed file b.cc
and the supplied target ImageUtil depends upon it. ImageUtil is not output as a build_target
...
def _GenerateTargets(data, target_list, target_dicts, toplevel_dir, files,
  ...
  . ImageUtil dictionary mapping from fully qualified name to Target.
  . ImageUtil list of the targets that have a source file in |files|.
